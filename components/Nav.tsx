...
// Slim top bar shown once a child profile exists. Mobile-first, large tap targets.
export function Nav({
  current,
  onNavigate,
}: {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-white/10 bg-twilight-900/70 backdrop-blur">
      <div className="mx-auto flex max-w-3xl items-center justify-between px-4 py-3">
        <button
          onClick={() => onNavigate('loop')}
          className="flex items-center gap-2 font-rounded text-2xl font-extrabold text-parchment"
        >
          <span>💛</span> Gleea
        </button>
          ...
          {tabs.map((t) => (
            <button
              ...
              className={
                ...
                (isActive(t.view)
                  ? 'bg-gold text-twilight-900 shadow-glow'
